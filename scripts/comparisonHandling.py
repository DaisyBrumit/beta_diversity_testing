...
rootdir = '/Users/dfrybrum/beta_diversity_testing/' #ref for laptop
studyList = ['Zeller', 'Jones', 'Vangay', 'Noguera-Julian'] # study names, also subdirs for rootdir
#studyList = ['Vangay', 'Noguera-Julian', 'Jones'] # study names, also subdirs for rootdir
...
for study in studyList:
    ...
    for root, dirs, files, in os.walk(rootdir+study+'/'):
        ...
        for file in files:
            ...
            if file.endswith('distance_matrix.tsv'):
                ...
                #string_elements = string_elements[:-2] # remove pcoa, results, ord.txt
                beta_method = "_".join(string_elements[:-2])  # now we can ID our current method
                ...
                ord = pcoa(dist, method='eigh', number_of_dimensions=3, inplace=False) # RUN IN FOR LOOP FOR MULTIPLE DIMS LATER
    ...
    accuracy_df.to_csv(rootdir + study + '/strat_accuracy_table.txt', sep='\t', index=False)
    roc_df.to_csv(rootdir + study + '/strat_roc_auc_table.txt', sep='\t', index=False)
