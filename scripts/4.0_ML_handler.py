...
for study in studyList:
    ...
    for root, dirs, files, in os.walk(rootdir + study + '/ordinations/'):
        ...
        for file in files:
            ...
            # ignore all high abundance ordinations
            else:
                continue
    ...
    accuracy_df.to_csv(rootdir + study + '/ML/accuracy_' + ML_method + '.tsv', sep='\t', index=False)
    roc_df.to_csv(rootdir + study + '/ML/roc_' + ML_method + '.tsv', sep='\t', index=False)
    r2_df.to_csv(rootdir + study + '/ML/r2_' + ML_method + '.tsv', sep='\t', index=False)
