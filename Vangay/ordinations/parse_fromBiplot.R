...
parse_ords <- function(filepath) {
  ...
  df <- tibble(PC1 = numeric(), PC2 = numeric(), PC3 = numeric())
  ...
  for (line in readLines(filepath)) {
    ...
    if (write == 0 && grepl("^Site", line)) {
      ...
      if (line == '') {
        ...
        index_value <- sub("\\.0$", '', line[1])  # Extract site value with no decimal
        
        index <- c(index, index_value)
        
        df <- bind_rows(df, tibble(PC1 = as.numeric(line[2]), PC2 = as.numeric(line[3]), PC3 = as.numeric(line[4])))
      }
    } else {
      next
    }
